...
@register_trainer("GraphClassificationFull_PPI")
class GraphClassificationFullTrainer_PPI(BaseGraphClassificationTrainer):
    """
    ...
    def get_task_name(cls):
        return "GraphClassification_PPI"
    ...
    def _train_only(self, train_loader, valid_loader=None):
        ...
        earlystop = EarlyStopping(mode="max", patience=self.patience)
        for epoch in range(1, self.max_epoch + 1):
            ...
            for data in train_loader:
                ...
                optimizer.zero_grad()
                output = model(data) ### multi-level gnn
                ...
                if self.lr_scheduler_type:
                    scheduler.step()
            val_metric = self._evaluate(valid_loader, self.feval)
            print("loss & acc",loss.item(), self._evaluate(train_loader, self.feval))
    ...
    # ppi
    def train(self, dataset, keep_valid_result=True, batch_size=1):
        """
        ...
        for i, fold in enumerate(dataset):
            train_loader=DataLoader(fold[0], batch_size, shuffle = True)
            valid_loader=DataLoader(fold[1], batch_size)
            self._train_only(train_loader, valid_loader)
            pred = self._predict_only(valid_loader)
            valid_result += pred.max(1)[1]
            valid_result_prob += pred
            valid_score += self.evaluate(dataset, mask="val", feval=self.feval)
        
        self.valid_result = valid_result / float(len(dataset))
        self.valid_result_prob = valid_result_prob / float(len(dataset))
        self.valid_score = valid_score / float(len(dataset))
        print("gclf_valid_score", self.valid_score)
    ...
    def evaluate(self, dataset, mask="val", feval=None):
        ...
        for i, fold in enumerate(dataset):
            if mask == "train":
                loader=DataLoader(fold[0], self.batch_size, shuffle = True) # train_loader

            elif mask == "test":
                loader=DataLoader(fold[2], self.batch_size) # test_loader

            elif mask == "val":
                loader=DataLoader(fold[1], self.batch_size) # val_loader
            evaluate += self._evaluate(loader, feval)
        return evaluate / float(len(dataset))
    ...
    def _evaluate(self, loader, feval=None):
        ...
        res = []
        for f in feval:
            res.append(f.evaluate(y_pred_prob.cpu().detach().numpy(), y_true.cpu().detach().numpy()))
        if return_signle:
            return res[0]
        return res
