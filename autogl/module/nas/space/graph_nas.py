...
class GraphNasGraphClassificationSpace2(BaseSpace):
    ...
    # 主要改这个！！！！
    def forward(self, data, batch): # batch 后面记得去掉！！
        # print("nas_1")
        # batch_size=max(batch).item()+1
        print(data) # Batch(batch=[578], edge_index=[2, 8458], ptr=[2], x=[578, 2], y=[1])
        # batch_size = 
        edge_index = self.construct_inner_edge(self.edge_index,batch_size,self.num_nodes[0])

        x = data.x
        # data_inner = 
        # data_cross = 
        # x = F.dropout(x, p=self.dropout, training=self.training)
        # node_in = getattr(self, f"in_{1}")(x)
        ...
        for layer in range(2, self.layer_number + 2):
            ...
            # cross_edge_index
            cross_edge_index=self.cross_edge_indexs[layer-2]
            cross_edge_index=self.construct_cross_edge(cross_edge_index,batch_size,num_nodes1,num_nodes2)
            # inner_edge_index
            inner_edge_index=self.inner_edge_indexs[layer-2]
            inner_edge_index=self.construct_inner_edge(inner_edge_index,batch_size,num_nodes2)

            # print("nas_12")
            node_in = getattr(self, f"in_{layer}")(x)
            op=getattr(self, f"op_{layer}")
            print(op) # GATConv(64, 64, heads=1)
            print(data) # Batch(batch=[578], edge_index=[2, 8458], ptr=[2], x=[578, 2], y=[1])
            print(node_in)
            node_out = bk_gconv(op,data,node_in) # op(data,node_in)
            prev_nodes_out.append(node_out)
    ...
    #         node_in = getattr(self, f"in_{layer}")(prev_nodes_out)
    #         op=getattr(self, f"op_{layer}")
    #         node_out = bk_gconv(op,data,node_in) # op(data,feat)
    #         prev_nodes_out.append(node_out)
    ...
    def construct_inner_edge(self, edge_index,batch_size,num_nodes):
    ...
    def construct_cross_edge(self, edge_index,batch_size,num_nodes1,num_nodes2):
