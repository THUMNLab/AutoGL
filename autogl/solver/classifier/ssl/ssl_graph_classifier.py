...
from .utils import _parse_hp_space, _parse_model_hp, _initialize_single_model
...
from ....module.train.ssl import BaseContrastiveTrainer
...
LOGGER = get_logger("SSLGraphClassifier")
BACKEND = DependentBackend.get_backend_name()

class SSLGraphClassifier(BaseClassifier):
    """
    ...
    def _init_graph_module(
        ...
        for i, model in enumerate(graph_models):
            # init the trainer
            if not isinstance(model, BaseContrastiveTrainer):
                trainer = (
            ...
            if self._model_hp_spaces is not None:
                if self._model_hp_spaces[i] is not None:
                    if isinstance(self._model_hp_spaces[i], dict):
                        ...
                        prediction_head_hp_space = self._model_hp_spaces[i].get('prediction_head', None)
                    else:
                    ...
                    if prediction_head_hp_space is not None:
                        trainer.prediction_head.hyper_parameter_space = prediction_head_hp_space
        ...
        if isinstance(trainer, dict):
            ...
            for i in range(len(model_list)):
                ...
                trainer_wrapper = TRAINER_DICT[default_trainer](
                    model=model, prediction_head=models[i]["prediction_head"].pop("name", None), **default_kwargs
                )
